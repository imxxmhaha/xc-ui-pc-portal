...
<script>
  ...
  export default {
    ...
    async asyncData({ store, route }) {//服务端调用方法
      ...
      // 向java服务端发起请求搜索课程
      // 1.搜索课程
      let course_data = await courseApi.search_course(page,2,route.query); // 返回promise对象
      // 2.查询分类
      let category_data = await courseApi.sysres_category();
      if (course_data &&　course_data.queryResult ) {
        // 全部分类
        let category = category_data.category; // 全部分类
        let first_category = category[0].children;//一级分类
        let second_category = [];//二级分类,根据一级分类动态显示
        let keywords = ''
        let mt=''
        let st=''
        let grade=''
        ...
        return {
          ...
          first_category: first_category,  // 一级分类
          second_category: second_category, // 二级分类
          keywords:keywords,
          mt:mt,
          st:st,
          grade:grade,
